...
### For individual_level calculations:
```{r}
...
```{r}
# save as r object
saveRDS(cleaned_predicted_probs_ind_tibble, file = "./data/shiny_ind_data.rds")
```


#### For days bad mental health outcome
```{r Bad mental health day outcome}
...
```{r}
# save as r object
saveRDS(cleaned_predicted_probs_ind_tibble2, file = "./data/shiny_ind_data2.rds")
```
...
```{r}
saveRDS(cleaned_pred_marg_means, file = "./data/shiny_group_data.rds")
```
...
```{r}
saveRDS(cleaned_pred_marg_means2, file = "./data/shiny_group_data2.rds")
```
...
ind_mh_days %>% 
    filter(sex_l == "male", 
           ...
           race_l == "white",
           age_l == "18-29",
           marital_status_l == "married",
           education_l == "less than high school",
           income_l == "<=35000",
           employment_l == "employed",
           general_health_l == "excellent",
           exercise_l == "yes") %>% 
...
```{r}
ind_depr %>% unnest(predict_output) %>% select(Predicted_Probability, SE)
